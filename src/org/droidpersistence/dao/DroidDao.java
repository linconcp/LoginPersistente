...
public abstract class DroidDao<T, ID extends Serializable> {
  ...
  public DroidDao(Class<T> model, TableDefinition<T> tableDefinition, SQLiteDatabase database) {
    ...
    setArrayColumns(this.tableDefinition.getArrayColumns());
    setTableName(this.tableDefinition.getTableName());
    setFieldDefinition(this.tableDefinition.getFieldDefinition());
    createInsertStatement(this.tableDefinition.getTableName(), tableDefinition.getArrayColumns());
    setIdColumn(this.tableDefinition.getPK());
    setListFieldModels(this.tableDefinition.getLIST_FIELD_MODEL());

    if (!getInsertStatement().trim().isEmpty()) {
  ...
  public boolean delete(ID id) {
    ...
    try {
      database.delete(getTableName(), getIdColumn() + " = ?", new String[] { String.valueOf(id) });
      result = true;
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public T get(ID id) {
    T object = null;
    Cursor cursor = database.query(getTableName(), getArrayColumns(), getIdColumn() + " = ?",
  ...
  public List<T> getAll() {
    ...
    Cursor cursor = database.query(getTableName(), getArrayColumns(), null, null, null, null, "1");
  ...
  public List<T> getAllbyClause(String clause, String[] clauseArgs, String groupBy, String having, String orderBy) {
    List<T> objectList = new ArrayList<T>();
    Cursor cursor = database.query(getTableName(), getArrayColumns(), clause, clauseArgs, groupBy, having, orderBy);
    if (cursor.moveToFirst()) {
      try {
        do {
          T object = buildDataFromCursor(cursor);
  ...
  public T getByClause(String clause, String[] clauseArgs) {
    T object = null;
    Cursor cursor = database.query(getTableName(), getArrayColumns(), clause, clauseArgs, null, null, "1");
    if (cursor.moveToFirst()) {
      try {
        object = buildDataFromCursor(cursor);
      } catch (Exception e) {
  ...
  public void update(T object, ID id) throws Exception {
    ...
    database.update(getTableName(), values, getIdColumn() + " = ?", new String[] { String.valueOf(id) });
  }
  ...
  public void update(T object, String clause, String[] clauseArgs) throws Exception {
    ...
    database.update(getTableName(), values, clause, clauseArgs);
  }
